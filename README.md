...
Сборка запускает интерфейс и API вместе. Для демонстрации откройте интерфейс, загрузите встроенный набор EKT, изучите рекомендации, затем оформите закуп и поступление в **Data Editor**.
...
1. Кнопка **«⚡️ Демо-кейс EKT»** — загрузка синтетического отраслевого набора с расчётом заказов и бюджета в тенге (₸).
2. Кнопка **«💥 Аномалии (Stress Test)»** — стресс-тест на 20 294 транзакциях с 8 смоделированными типами коммерческих аномалий (тендеры, скачки цен меди, ЧС/шторм, ошибки операторов 1С, дефициты, неликвиды, каннибализация).
