# basqar (StockPilot) — объяснимое пополнение запасов
...
1. Кнопка **«⚡️ ekt.kz Dataset (Казахстан)»** — мгновенная загрузка синтетического демонстрационного датасета EKT с расчётом заказов и бюджета в тенге (₸).
...
4. Раздел **«Данные → Загрузка файлов»** — загрузка файлов по отдельности или книги Excel целиком (`ekt_sales_and_stock_history.xlsx` или `ekt_extreme_anomalies_sales_and_stock.xlsx`, 5 листов в 1 клик).
5. Раздел **«Данные → Редактор данных»** — операции через сайт: закуп, продажа, поступление, перемещение, корректировка и возврат. Несколько товаров проводятся одним документом; вкладка «Склад» показывает все SKU из истории проекта, включая позиции с нулевым остатком. «Справочники» сохраняют точечное редактирование таблиц и SQLite-черновик.

### Операции с товарами без повторного Excel

1. Загрузите книгу через **Data intake**, проверьте и утвердите рекомендации.
2. В **Data editor → Закуп** добавьте утверждённые рекомендации или вручную наберите несколько SKU, укажите поставщика, склад и дату поставки. Заказ записывается в журнал и увеличивает **товары в пути**, но ещё не физический остаток.
...
StockPilot реализует пять направлений Must Have. Проверки и ограничения описаны ниже:
...
| `POST` | `/api/data/load-ekt` | Загрузка и расчёт синтетического демонстрационного датасета EKT |
...
## Оригинальные архивы партнёров и ограничения

В **Данные → Загрузка файлов** добавлен отдельный импорт ZIP Systeme Electric и IEK (`POST /api/data/upload-partner`). Выберите архив одного поставщика и явно укажите Lead Time от 1 до 365 дней: в исходных книгах подтверждённого общего срока нет. Дата среза необязательна; по умолчанию используется последняя дата продаж. Импорт заменяет текущий набор, показывает количество строк, соответствие кодов артикулам, предупреждения и происхождение данных.

Встроенные EKT и стресс-наборы **синтетические**, а не оригинальные продажи компании. Предоставленные коммерческие архивы не публикуются в Git. Адаптер сохраняет внутренний код отдельно от артикула поставщика, возвраты и даты партий. Месячные продажи не добавляются второй раз к дневной истории. Неизвестные цены и клиенты явно помечаются; они не подменяются вымышленными наблюдениями.

Месячный начальный остаток не является текущим запасом. Такие строки помечаются устаревшими; утверждение блокируется до подтверждения текущего количества в **Данные → Справочники → Остатки**. Точные ежедневные stockout из месячных снимков не выдумываются. При неизвестных ценах бюджет считается неполным. Перед реальным заказом уточните цены, упаковки и условия поставщика.

Текущий MVP рассчитан на **один backend-процесс в локальном контуре**. Изменения этого процесса согласуются общей блокировкой и транзакцией SQLite; многопроцессная работа, авторизация, роли и промышленное резервное восстановление пока не реализованы. Не публикуйте API как защищённую систему в Интернет. Большие архивы обрабатываются синхронно. Прямая интеграция с1С и автоматическая отправка поставщику отсутствуют; утверждение в приложении не отправляет заказ.

## Проверка исправлений

```sh
python -m pytest qa backend/tests -q
python qa/backtest.py
cd frontend
npx tsc --noEmit
node --test tests/editor-regressions.test.mjs
npm run build
```

Новые регрессии проверяют рестарт с пустой историей, конкурентные правки, постоянные ID строк, идемпотентность, импорт собственного Excel, регулярные B2B-покупки, хвостовой stockout, частичные поставки и черновики интерфейса. Критерии: [AUDIT_REMEDIATION.md](qa/AUDIT_REMEDIATION.md).

`qa/backtest.py` сравнивает прогноз со средним за 28 дней на временных отсечениях синтетических рядов, включая шум. На шумных данных преимущество не гарантировано. Это диагностическая проверка, **не доказательство промышленной точности или экономии денег**. Формальная сдача также требует отдельного действия на платформе хакатона; GitHub push его не заменяет.
