...
4. Кнопка **«⚠️ Stress anomalies»** — стресс-сценарий с реальными коммерческими аномалиями, stockout-периодами и экстремальными транзакциями для демонстрации устойчивости алгоритма.
...
| `POST` | `/api/data/load-ekt-extreme` | Загрузка и расчёт стресс-датасета с бизнес-аномалиями |
