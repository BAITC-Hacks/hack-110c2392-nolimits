...
export default function DataEditor({ onToast, onRefresh, recommendations }: { onToast: (message: string) => void; onRefresh: () => void; recommendations: Recommendation[] }) {
  ...
  return <section className="operations-page">
    <div className="page-intro operations-intro"><div><p className="eyebrow">DATA EDITOR / ОПЕРАЦИИ</p><h2>Управление товарами</h2><p>Оформляйте несколько товаров как один документ. Остатки, товары в пути, история продаж и рекомендации обновятся автоматически.</p></div><a className="button primary" href={inventoryExportUrl()}>↓ Скачать обновлённый Excel</a></div>
    <div className="operations-sections">{sections.map(item => <button key={item.key} className={section === item.key ? 'operations-section active' : 'operations-section'} onClick={() => { if (item.key !== section) { if (lines.length && !window.confirm('Несохранённые позиции документа будут очищены. Продолжить?')) return; setLines([]); setEntry(newLine()); setPartner(''); setDestination(''); setRequestId(crypto.randomUUID()) }; setSection(item.key) }}><strong>{item.label}</strong><small>{item.help}</small></button>)}</div>

    {section === 'warehouse' && <div className="content-card"><div className="card-header"><div><p className="eyebrow">СКЛАД / КАТАЛОГ</p><h3>Все товары в истории проекта <span>{catalog.length}</span></h3></div><div className="search editor-search"><span>⌕</span><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Артикул, название, склад" /></div></div><div className="table-wrap"><table><thead><tr><th>Артикул</th><th>Товар</th><th>Категория</th><th>Склад</th><th>В наличии</th><th>В пути</th></tr></thead><tbody>{visibleStock.map((row, index) => <tr key={`${row.sku}-${row.warehouse}-${index}`}><td><strong className="sku">{row.sku}</strong></td><td>{row.product_name}</td><td>{row.category}</td><td>{row.warehouse || 'Пока не размещён'}</td><td><strong>{fmt(row.current_stock)}</strong></td><td>{fmt(row.in_transit)}</td></tr>)}</tbody></table>{!visibleStock.length && <div className="empty">Товары не найдены.</div>}</div></div>}

    {section === 'reference' && <RawDataEditor onToast={onToast} onRefresh={() => { reload(); onRefresh() }} />}

    {section === 'movements' && <div className="movement-subtabs">{movementKinds.map(item => <button key={item.key} className={movementTab === item.key ? 'active' : ''} onClick={() => { if (item.key !== movementTab && lines.length && !window.confirm('Несохранённые позиции документа будут очищены. Продолжить?')) return; setMovementTab(item.key); setLines([]); setRequestId(crypto.randomUUID()) }}>{item.label}</button>)}</div>}
    {section === 'movements' && movementTab === 'HISTORY' && <div className="content-card"><div className="card-header"><div><p className="eyebrow">ЖУРНАЛ</p><h3>Движения товаров <span>{history.length}</span></h3></div><a className="button subtle" href={inventoryExportUrl()}>↓ Excel</a></div><div className="table-wrap"><table><thead><tr><th>Дата</th><th>Тип</th><th>Склад</th><th>Контрагент / документ</th><th>Позиции</th></tr></thead><tbody>{history.map(item => <tr key={item.id}><td>{item.date}</td><td><strong>{kindLabel[item.kind]}</strong></td><td>{item.warehouse}{item.destination_warehouse ? ` → ${item.destination_warehouse}` : ''}</td><td>{item.partner || item.reference || '—'}</td><td>{item.lines.map(line => `${line.sku} × ${fmt(line.quantity)}`).join(', ')}</td></tr>)}</tbody></table>{!history.length && <div className="empty">Операций пока нет.</div>}</div></div>}
    ...
    {(section === 'purchase' || section === 'sale' || (section === 'movements' && movementTab !== 'HISTORY')) && <div className="operations-layout">
      <div className="operations-main">
        ...
        <div className="content-card operations-card"><div className="card-header"><div><p className="eyebrow">КАК НА КАССЕ</p><h3>Добавить товар</h3></div></div><div className="operations-fields line-fields">
