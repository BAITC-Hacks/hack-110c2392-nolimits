...
export default function DataEditor({ onToast, onRefresh, recommendations, initialReference = false }: { onToast: (message: string) => void; onRefresh: () => void; recommendations: Recommendation[]; initialReference?: boolean }) {
  const { formatNumber: fmt, formatDate } = useI18n()
  const fmtMoney = fmt
  ...
  const addLine = () => {
    if (!entry.sku.trim() || !entry.product_name.trim()) { onToast(translateText('Укажите артикул и название товара')); return }
    if (!Number.isFinite(entry.quantity) || entry.quantity === 0 || (entry.quantity < 0 && kind !== 'ADJUSTMENT')) { onToast(translateText('Проверьте количество')); return }
    const unitPrice = parseUnitPrice(priceInput)
    if (unitPrice === null) { onToast(translateText('Укажите корректную цену, например 1500 или 1500,50')); return }
  ...
  const addRecommendation = (row: Recommendation) => {
    if (lines.some(line => line.recommendation_id === row.id)) { onToast(translateText('Эта рекомендация уже добавлена')); return }
    if (lines.length && (warehouse !== row.warehouse || partner !== row.supplier_name)) { onToast(translateText('В одном заказе должен быть один поставщик и склад')); return }
  ...
  const submit = async () => {
    if (!lines.length) { onToast(translateText('Добавьте товары в операцию')); return }
    ...
    try {
      ...
      storeMovementDraft({ section, movementTab, date, warehouse, destination, partner, reference: '', arrival, entry: newLine(), priceInput: '', lines: [], requestId: nextRequestId })
      setLines([]); setEntry(newLine()); setPriceInput(''); setReference(''); setRequestId(nextRequestId)
      await reload(); onRefresh()
      onToast(translateText('Операция сохранена: {count} позиций. Excel обновлён.', { count: result.movement.lines.length }))
    ...
    {(section === 'purchase' || section === 'sale' || (section === 'movements' && movementTab !== 'HISTORY')) && <div className="operations-layout">
      <div className="operations-main">
        <div className="content-card operations-card operations-document"><div className="card-header"><div><p className="eyebrow">{translateText("НОВЫЙ ДОКУМЕНТ")}</p><h3>{translateText(kindLabel[kind])}</h3></div><span className="editor-status">{lines.length} {translateText("поз.")}</span></div><div className="operations-fields">
          ...
          <label className="editor-field"><span>{translateText(kind === 'TRANSFER' ? 'Откуда *' : 'Склад *')}</span><input list="warehouses" value={warehouse} onChange={event => setWarehouse(event.target.value)} placeholder={translateText("Выберите склад")} /></label>
          ...
          {(kind === 'PURCHASE' || kind === 'SALE' || kind === 'RETURN') && <label className="editor-field"><span>{translateText(kind === 'PURCHASE' ? 'Поставщик *' : 'Покупатель')}</span><input value={partner} onChange={event => setPartner(event.target.value)} placeholder={translateText(kind === 'PURCHASE' ? 'Название поставщика' : 'ID покупателя')} /></label>}
      ...
      <aside className="operations-aside">{section === 'purchase' && <div className="content-card operations-card"><div className="card-header"><div><p className="eyebrow">{translateText("ИЗ РЕКОМЕНДАЦИЙ")}</p><h3>{translateText("Утверждённые заказы")}<span>{approved.length}</span></h3></div></div><div className="recommendation-picker">{approved.slice(0, 80).map(row => <button key={row.id} onClick={() => addRecommendation(row)}><span><strong>{row.sku}</strong><small>{row.product_name}<br />{row.supplier_name} · {row.warehouse}</small></span><b>＋ {fmt(row.final_quantity)}</b></button>)}{!approved.length && <p>{translateText("Утвердите рекомендацию на главной странице или добавьте товар вручную.")}</p>}</div></div>}
        <div className="content-card operations-card"><div className="card-header"><div><p className="eyebrow">{translateText("ПОСЛЕДНИЕ ДЕЙСТВИЯ")}</p><h3>{translateText("Журнал")}</h3></div></div><div className="operations-recent">{history.slice(0, 6).map(item => <div key={item.id}><strong>{translateText(kindLabel[item.kind])}</strong><span>{item.date} · {item.lines.length} {translateText('поз.')}</span></div>)}{!history.length && <p>{translateText("Пока нет операций.")}</p>}</div></div>
      </aside>
    </div>}
  </fieldset></section>
}
