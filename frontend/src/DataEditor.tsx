  ...
  return <section className="operations-page">
    ...
    {(section === 'purchase' || section === 'sale' || (section === 'movements' && movementTab !== 'HISTORY')) && <div className="operations-layout">
      <div className="operations-main">
        <div className="content-card operations-card"><div className="card-header"><div><p className="eyebrow">{translateText("НОВЫЙ ДОКУМЕНТ")}</p><h3>{kindLabel[kind]}</h3></div><span className="editor-status">{lines.length} позиций</span></div><div className="operations-fields">
        ...
        <div className="content-card operations-card"><div className="card-header"><div><p className="eyebrow">{translateText("СОСТАВ ДОКУМЕНТА")}</p><h3>{translateText("Добавить товар")}</h3></div></div><div className="operations-fields line-fields">
          ...
          <label className="editor-field"><span>{translateText("Цена за единицу, ₸")}</span><input type="number" min="0" step="any" value={entry.unit_price} onChange={event => setEntry(current => ({ ...current, unit_price: Number(event.target.value) }))} /></label>
