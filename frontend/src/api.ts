...
export async function uploadFile(dataset: string, file: File) { const form = new FormData(); form.append('file', file); return request<{ rows_loaded: number; errors: string[]; warnings: string[] }>(`/api/data/upload/${dataset}`, { method: 'POST', body: form }) }
export async function uploadWorkbook(file: File) { const form = new FormData(); form.append('file', file); return request<{ message: string; datasets: Record<string, number>; recommendations: number }>('/api/data/upload-workbook', { method: 'POST', body: form }) }
