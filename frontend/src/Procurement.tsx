...
export default function Procurement({ rows, summary, selectedId, onOpen, busy }: { rows: Recommendation[]; summary: Summary; selectedId?: string; onOpen: (row: Recommendation) => void; busy: boolean }) {
  ...
  return <>
    <section className="kpi-grid" aria-label={translateText("Показатели всего расчёта")}>
      ...
      <article className="kpi"><span>{translateText("Бюджет")}</span><strong>{summary.total_budget_kzt == null ? 'Не рассчитан' : money(summary.total_budget_kzt)}</strong><small>{rows.some(r => r.final_quantity > 0 && r.total_cost_kzt == null) ? 'неполный · есть позиции без цены' : 'расчётная стоимость закупки'}</small></article>
    ...
    <section className="content-card" aria-label={translateText("План закупок")} aria-busy={busy}>
      ...
      <div className="search-row"><label className="search"><Icon name="search"/><input aria-label={translateText("Поиск по названию или SKU")} type="search" value={search} onChange={e => { setSearch(e.target.value); setPage(0) }} placeholder={translateText("Поиск по названию или SKU")}/></label><button className="button subtle mobile-filter" aria-expanded={filtersOpen} aria-controls="filters" onClick={() => setFiltersOpen(v => !v)}><Icon name="settings"/>Фильтры{activeFilters ? ` · ${activeFilters}` : ''}</button></div>
      ...
      {!grouped && <div className="pagination"><span>{visible.length ? `${currentPage * 25 + 1}–${Math.min((currentPage + 1) * 25, visible.length)} из ${visible.length}` : '0 позиций'}<small>{translateText("Количество — в единицах исходных данных")}</small></span><div><button className="button subtle" disabled={currentPage === 0} onClick={() => setPage(currentPage - 1)}>{translateText("Назад")}</button><button className="button subtle" disabled={(currentPage + 1) * 25 >= visible.length} onClick={() => setPage(currentPage + 1)}>{translateText("Далее")}</button></div></div>}
    </section>
    <details className="processing-note"><summary>{translateText("Дополнительная аналитика расчёта")}</summary><p>Выбросов обработано: {summary.detected_anomalies}. Восстановленный спрос: {number(summary.estimated_lost_demand)}. Объём заказа из сводки API: {number(summary.total_recommended_units)}. Эти показатели относятся ко всему расчёту, независимо от фильтров.</p></details>
  </>
}
