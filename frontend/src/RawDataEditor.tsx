...
function displayValue(value: unknown, yes: string, no: string, formatNumber: (value: number) => string, formatDate: (value: string) => string): string {
  if (value === null || value === undefined) return '—'
  if (typeof value === 'boolean') return value ? yes : no
  if (typeof value === 'number') return formatNumber(value)
  if (typeof value === 'string' && /^\d{4}-\d{2}-\d{2}(?:T|$)/.test(value) && Number.isFinite(Date.parse(value))) return formatDate(value)
