...
export default function RawDataEditor({ onToast, onRefresh }: { onToast: (message: string) => void; onRefresh: () => void }) {
  ...
  return <section className="editor-page">
    <div className="page-intro editor-intro"><div><p className="eyebrow">ИСХОДНЫЕ ДАННЫЕ</p><h2>Справочники и данные</h2><p>Создавайте товары, поставщиков и операции прямо в StockPilot. Черновик сохраняется автоматически.</p></div><div className="editor-status">{restored ? '↩ Черновик восстановлен' : 'Изменения сохраняются на сервере'}</div></div>
