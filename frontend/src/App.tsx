...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><div className="live-dot"><i /> Engine online</div><small>v1.2 · ekt.kz + anomalies</small></div>
    </aside>
...
function Imports({ onToast, onRefresh, onEkt, onAnomalies }: { onToast: (message: string) => void; onRefresh: () => void; onEkt: () => void; onAnomalies: () => void }) {
  ...
  return <section className="import-page"><div className="page-intro"><div><p className="eyebrow">DATA FOUNDATION</p><h2>Bring your signals together.</h2><p>Upload clean operational data or full ekt.kz multi-sheet workbook. Supports both Russian and English headers.</p></div><div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}><button className="button subtle" onClick={async () => { await loadDemo(); await calculate(); onRefresh(); onToast('Demo dataset loaded') }}>✦ Load synthetic demo</button><button className="button primary" onClick={onEkt}>⚡️ Load ekt.kz Dataset (ТОО «Электрокомплект»)</button><button className="button" style={{ background: '#f59e0b', color: '#000', fontWeight: 600 }} onClick={onAnomalies}>💥 Load Extreme Anomalies Dataset</button></div></div>
