...
function Anomalies({ onOpen }: { onOpen: (row: Recommendation) => void }) {
  ...
  return <section className="content-card anomaly-page"><div className="card-header"><div><p className="eyebrow">AUDIT TRAIL</p><h3>Detected anomalies <span>{items.length}</span></h3><p className="subcopy">Large one-off transactions are excluded from regular demand, never deleted.</p></div></div><div className="table-wrap"><table className="anomaly-table" role="table" aria-label="Detected anomalies"><thead role="rowgroup"><tr role="row"><th role="columnheader" scope="col">Date</th><th role="columnheader" scope="col">SKU / warehouse</th><th role="columnheader" scope="col">Customer</th><th role="columnheader" scope="col">Quantity</th><th role="columnheader" scope="col">Typical</th><th role="columnheader" scope="col">Reason</th></tr></thead><tbody role="rowgroup">{items.map((item, index) => <tr key={index} role="row"><td role="cell" data-label="Date">{item.date}</td><td role="cell" data-label="SKU / warehouse"><strong>{item.sku}</strong><span className="cell-muted">{item.warehouse}</span></td><td role="cell" data-label="Customer">{item.customer_id}</td><td role="cell" data-label="Quantity"><strong className="danger-text">{money(item.quantity)}</strong></td><td role="cell" data-label="Typical">{money(item.typical_quantity)}</td><td role="cell" data-label="Reason"><span className="reason-pill">{item.reason}</span></td></tr>)}</tbody></table>{!items.length && <div className="empty">No anomalies detected yet. Run the demo scenario to populate the audit trail.</div>}</div></section>
}
