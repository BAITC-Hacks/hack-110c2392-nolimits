...
import { adjustOrder, approveOrder, calculate, exportUrl, getAnalytics, getOutliers, getRecommendations, loadDemo, loadEkt, uploadFile, uploadWorkbook } from './api'
...
export default function App() {
  ...
  const visibleRows = useMemo(() => rows.filter(row => (urgency === 'ALL' || row.urgency === urgency) && (warehouse === 'ALL' || row.warehouse === warehouse) && (supplier === 'ALL' || row.supplier_id === supplier) && (category === 'ALL' || row.category === category) && (!search || `${row.sku} ${row.product_name}`.toLowerCase().includes(search.toLowerCase()))).sort((a, b) => sort === 'urgency' ? urgencyOrder[a.urgency] - urgencyOrder[b.urgency] : sort === 'days_of_cover' ? a.days_of_cover - b.days_of_cover : b.recommended_quantity - a.recommended_quantity), [rows, urgency, warehouse, supplier, category, search, sort])
  ...
  return <div className="app-shell">
    ...
    <main className="main"><header className="topbar"><div><p className="eyebrow">PURCHASING / WAREHOUSE CONTROL</p><h1>{page === 'overview' ? 'Replenishment Cockpit' : nav.find(item => item.key === page)?.label}</h1></div><div className="top-actions"><span className="date-chip">● Live data · {new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span><button className="button subtle" onClick={demo} disabled={busy}>↻ Synthetic demo</button><button className="button primary" onClick={ekt} disabled={busy}>⚡️ ekt.kz Dataset (Казахстан)</button></div></header>
      ...
      {page === 'imports' && <Imports onToast={showToast} onRefresh={refresh} onEkt={ekt} />}
      {page === 'anomalies' && <Anomalies onOpen={openSku} />}
    </main>
    {selected && <Detail row={selected} analytics={analytics} onClose={() => setSelected(null)} onError={showToast} onChanged={async row => { setRows(current => current.map(item => item.id === row.id ? row : item)); setSelected(row); await refresh(); showToast(row.status === 'APPROVED' ? 'Order approved' : 'Quantity adjusted') }} />}
...
function SupplierGroups({ rows, onOpen }: { rows: Recommendation[]; onOpen: (row: Recommendation) => void }) {
  ...
  return <section className="supplier-grid"><div className="supplier-section-title"><div><p className="eyebrow">ORDER PACKETS</p><h3>Grouped by supplier</h3></div><span>Review before approval</span></div>{groups.map(([id, items]) => {
...
function Imports({ onToast, onRefresh, onEkt }: { onToast: (message: string) => void; onRefresh: () => void; onEkt: () => void }) {
  ...
  const handleWorkbook = async (file?: File) => { if (!file) return; try { const response = await uploadWorkbook(file); onToast(`Excel workbook loaded: ${response.recommendations} recommendations calculated!`); onRefresh() } catch (error) { onToast(error instanceof Error ? error.message : 'Workbook upload failed') } }

  return <section className="import-page"><div className="page-intro"><div><p className="eyebrow">DATA FOUNDATION</p><h2>Bring your signals together.</h2><p>Upload clean operational data or full ekt.kz multi-sheet workbook. Supports both Russian and English headers.</p></div><div style={{ display: 'flex', gap: '8px' }}><button className="button subtle" onClick={async () => { await loadDemo(); await calculate(); onRefresh(); onToast('Demo dataset loaded') }}>✦ Load synthetic demo</button><button className="button primary" onClick={onEkt}>⚡️ Load ekt.kz Dataset (ТОО «Электрокомплект»)</button></div></div>
