...
export default function App() {
  ...
  const run = async (action: () => Promise<unknown>) => { setBusy(true); try { await action(); await refresh(); showToast(translateText('Данные обновлены')) } catch (e) { setError(e instanceof Error ? e.message : 'Не удалось выполнить действие') } finally { setBusy(false) } }
  ...
  return <div className={`app-shell ${collapsed ? 'sidebar-collapsed' : ''}`}>
    <aside className="sidebar"><div className="brand"><span className="brand-mark">B</span><span>basqar<small>{translateText("Управление закупками")}</small></span></div>{navigation}<div className="sidebar-bottom">{translateText("Рабочее пространство")}<small>{translateText("Запасы и пополнение")}</small></div><button className="nav-item sidebar-collapse" aria-label={collapsed ? t('sidebarShow') : t('sidebarHide')} title={collapsed ? t('sidebarShow') : t('sidebarHide')} onClick={() => setCollapsed(v => !v)}><Icon name={collapsed ? 'panelShow' : 'panelHide'}/><span>{collapsed ? t('sidebarShow') : t('sidebarHide')}</span></button></aside>
    <main className="main"><header className="topbar"><div className="heading"><button className="icon-button mobile-menu" aria-label={translateText("Открыть меню")} onClick={() => setMobileNav(true)}><Icon name="menu"/></button><div><h1>{translateText(current.label)}</h1><p>{translateText(current.description)}</p></div></div><div className="actions"><button className="button primary" disabled={busy} onClick={() => run(calculate)}><Icon name="refresh"/>{translateText(busy ? 'Загрузка…' : 'Пересчитать')}</button><button className="button subtle" disabled={!rows.length} onClick={() => setExportOpen(true)}><Icon name="export"/>{translateText("Экспорт")}</button></div></header>
      <div className="context-line">{translateText('Все склады')} · {translateText('данные на')} {summary.data_as_of ? formatDate(summary.data_as_of) : translateText('дату источника')}</div>
    ...
    {selected && <ItemDetail key={selected.id} row={selected} points={points} loading={analyticsLoading} analyticsError={analyticsError} onClose={() => { setSelected(null); requestId.current++ }} onChanged={r => { setSelected(r); setRows(old => old.map(item => item.id === r.id ? r : item)); void refresh(); showToast(translateText(r.status === 'APPROVED' ? 'Позиция утверждена' : 'Количество сохранено')) }}/>}
    {exportOpen && <Modal title={translateText("Экспорт заказов")} onClose={() => setExportOpen(false)}><div className="panel-body"><p>{translateText('Экспортируются все {count} рекомендаций текущего расчёта. Фильтры таблицы не применяются.', { count: rows.length })}</p><div className="actions"><a className="button primary" href={exportUrl('xlsx')}>{translateText("Скачать XLSX")}</a><a className="button subtle" href={exportUrl('csv')}>{translateText("Скачать CSV")}</a></div></div></Modal>}
