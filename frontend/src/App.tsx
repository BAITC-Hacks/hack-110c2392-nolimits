...
export default function App() {
  ...
  return <div className={sidebarCollapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
    ...
    {selected && <Detail row={selected} analytics={analytics} onClose={() => setSelected(null)} onError={showToast} onChanged={async row => { setRows(current => current.map(item => item.id === row.id ? row : item)); setSelected(row); await refresh(); showToast(row.status === 'APPROVED' ? t('orderApproved') : t('quantityAdjusted')) }} />}
...
function Detail({ row, analytics, onClose, onChanged, onError }: { row: Recommendation; analytics: { points: Point[]; outliers: unknown[] } | null; onClose: () => void; onChanged: (row: Recommendation) => void; onError: (message: string) => void }) {
  ...
  const save = async () => { setSaving(true); try { onChanged(await adjustOrder(row.id, quantity)) } catch (error) { onError(error instanceof Error ? error.message : t('couldNotSave')) } finally { setSaving(false) } }
  const approve = async () => { setSaving(true); try { onChanged(await approveOrder(row.id)) } catch (error) { onError(error instanceof Error ? error.message : t('couldNotSave')) } finally { setSaving(false) } }
