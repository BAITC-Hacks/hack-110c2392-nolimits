...
export default function ItemDetail({ row, points, loading, analyticsError, onClose, onChanged }: { row: Recommendation; points: Point[]; loading: boolean; analyticsError: string; onClose: () => void; onChanged: (row: Recommendation) => void }) {
  ...
  const close = () => { if (!saving && (!dirty || window.confirm(translateText('Закрыть без сохранения количества?')))) onClose() }
  ...
  const staleBalance = row.metadata.balance_is_stale === true
  const warningCopy: Record<string, string> = {
    'Stock balance is stale; enter the current available balance before approving this recommendation.': 'Утверждение недоступно: обновите фактический остаток в «Данные» → «Справочники» → «Остатки».',
    'Purchase cost is unknown; zero is a placeholder, not a confirmed supplier price.': 'Закупочная цена неизвестна. Ноль в источнике не является подтверждённой ценой поставщика.',
    'Customer identity is missing: client-level outlier detection is unavailable for those sales; demand is retained.': 'Нет идентификатора покупателя: аномалии по клиенту не определяются, спрос сохранён.',
  }
  const qualityWarnings = Array.isArray(row.metadata.data_quality_warnings) ? row.metadata.data_quality_warnings.filter((value): value is string => typeof value === 'string') : []
  return <Modal title={translateText("Карточка позиции")} className="item-drawer" onClose={close}>
    <div className="drawer-body">
      {(staleBalance || qualityWarnings.length > 0) && <section className="quality-notice" aria-label={translateText('Качество исходных данных')}>
        <h3>{translateText('Качество исходных данных')}</h3>
        {staleBalance && <p className="error">{translateText('Утверждение недоступно: обновите фактический остаток в «Данные» → «Справочники» → «Остатки».')}</p>}
        {!!qualityWarnings.length && <ul>{qualityWarnings.map((message, index) => <li key={index}>{translateText(warningCopy[message] || message)}</li>)}</ul>}
      </section>}
      <section><div className="badge-row"><Badge value={row.urgency}/><Badge value={row.status}/></div><h2 className="product-title">{row.product_name}</h2><p>{row.sku} · {warehouseName(row.warehouse)}</p><p>{row.supplier_name} · {row.lead_time_days} {translateText('дн. поставки')}</p></section>
      <section><h3>{translateText("Рекомендация")}</h3><dl className="detail-grid">{[['К заказу', `${number(row.recommended_quantity)} ${translateText('ед.')}`], ['Стоимость итогового заказа', money(row.final_quantity > 0 && (row.metadata.cost_unknown === true || !(row.unit_cost && row.unit_cost > 0)) ? null : row.total_cost_kzt)], ['Остаток', number(row.current_stock)], ['В пути', number(row.in_transit)], ['Прогноз за срок поставки', number(row.forecast_lead_time)], ['Страховой запас', number(row.safety_stock)]].map(([label, value]) => <div key={translateText(label)}><dt>{translateText(label)}</dt><dd>{value}</dd></div>)}</dl></section>
    ...
    <div className="drawer-footer"><div><button className="button subtle" disabled={saving || !dirty || !valid} onClick={() => change(false)}>{translateText("Сохранить")}</button><button className="button primary" disabled={saving || dirty || !valid || staleBalance || row.status === 'APPROVED'} onClick={() => change(true)}>{translateText(saving ? 'Сохраняем…' : row.status === 'APPROVED' ? 'Утверждено' : 'Утвердить')}</button></div><small>{translateText(dirty ? 'Сохраните новое количество перед утверждением.' : 'Утверждение не отправляет заказ поставщику.')}</small></div>
  </Modal>
}
