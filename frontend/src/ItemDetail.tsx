...
export default function ItemDetail({ row, points, loading, analyticsError, onClose, onChanged }: { row: Recommendation; points: Point[]; loading: boolean; analyticsError: string; onClose: () => void; onChanged: (row: Recommendation) => void }) {
  ...
  return <Modal title={translateText("Карточка позиции")} className="item-drawer" onClose={close}>
    <div className="drawer-body">
      ...
      <section><h3>{translateText("Рекомендация")}</h3><dl className="detail-grid">{[['К заказу', `${number(row.recommended_quantity)} ${translateText('ед.')}`], ['Стоимость итогового заказа', money(row.total_cost_kzt)], ['Остаток', number(row.current_stock)], ['В пути', number(row.in_transit)], ['Прогноз за срок поставки', number(row.forecast_lead_time)], ['Страховой запас', number(row.safety_stock)]].map(([label, value]) => <div key={translateText(label)}><dt>{translateText(label)}</dt><dd>{value}</dd></div>)}</dl></section>
    ...
    <div className="drawer-footer"><div><button className="button subtle" disabled={saving || !dirty || !valid} onClick={() => change(false)}>{translateText("Сохранить")}</button><button className="button primary" disabled={saving || dirty || !valid || row.status === 'APPROVED'} onClick={() => change(true)}>{translateText(saving ? 'Сохраняем…' : row.status === 'APPROVED' ? 'Утверждено' : 'Утвердить')}</button></div><small>{translateText(dirty ? 'Сохраните новое количество перед утверждением.' : 'Утверждение не отправляет заказ поставщику.')}</small></div>
  </Modal>
}
